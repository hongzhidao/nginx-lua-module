...
static const u_char *ngx_sha2_body(ngx_sha2_t *ctx, const u_char *data,
    size_t size);


void
ngx_sha2_init(ngx_sha2_t *ctx)
{
    ctx->a = 0x6a09e667;
    ctx->b = 0xbb67ae85;
    ctx->c = 0x3c6ef372;
...
void
ngx_sha2_update(ngx_sha2_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        size -= free;
        (void) ngx_sha2_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = ngx_sha2_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
void
ngx_sha2_final(u_char result[32], ngx_sha2_t *ctx)
{
    size_t  used, free;
...
static const u_char *
ngx_sha2_body(ngx_sha2_t *ctx, const u_char *data, size_t size)
{
